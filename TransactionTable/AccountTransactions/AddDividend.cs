...
namespace PortfolioPerformance_TableHelper.TransactionTable.AccountTransactions
{
    ...
    {
        public void AddDividend(DateTime DateTime, string SecurityName, string CashAccount,  double GrossValue, double Shares=-1,
            double Fees = -1, double Tax = -1, string WKN = "", string ISIN = "", string Symbol = "", string Note = "")
        {
            int index = MyTable.AppendEmptyRecord();
            MyTable.SetCell(TableHeaders.Type.Name, index, TransactionTypes.Dividend.Name);
